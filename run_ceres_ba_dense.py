...
def main():
    ...
    img_names = [img.name for _, img in img_items]
    ...
    # === Output: Ceres sparse model ===
    recon.write(out_dir)
    shutil.rmtree(tmp, ignore_errors=True)
    print(f"Saved Ceres model to {out_dir}/ ({recon.num_points3D()} points)")

    # === Dense point cloud from best K images (min pose change + spatially diverse) ===
    K_DENSE = 10
    from scipy.spatial.transform import Rotation as Rot
    pose_changes = []
    for i in range(N):
        if recon.exists_image(i + 1) and recon.image(i + 1).has_pose:
            cfw = recon.image(i + 1).cam_from_world()
            R_opt_i = cfw.rotation.matrix()
            dR = np.arccos(np.clip((np.trace(ext_all[i,:3,:3].T @ R_opt_i) - 1) / 2, -1, 1)) * 180 / np.pi
            pose_changes.append((dR, i))
    pose_changes.sort()
    # Select top K_DENSE with spatial diversity
    centers_opt = np.zeros((N, 3))
    for i in range(N):
        if recon.exists_image(i + 1) and recon.image(i + 1).has_pose:
            cfw = recon.image(i + 1).cam_from_world()
            R_cw = cfw.rotation.matrix(); t_cw = cfw.translation
            centers_opt[i] = -R_cw.T @ t_cw
    selected = []
    min_dist = scene_radius / (K_DENSE * 2)
    selected = []
    for dR, i in pose_changes:
        if len(selected) >= K_DENSE: break
        too_close = any(np.linalg.norm(centers_opt[i] - centers_opt[j]) < min_dist for j in selected)
        if not too_close:
            selected.append(i)
    selected.sort()
    print(f"\nDense cloud from {len(selected)} images (min pose change, spatially diverse):")
    pc_dict = {idx: dr for dr, idx in pose_changes}
    for i in selected:
        print(f"  {img_names[i]:25s}  dR={pc_dict.get(i, 0):.2f}°")

    # Generate dense point cloud
    STRIDE_DENSE = 4; CONF_THRESH = 3.0
    K_use = K_all
    H_use, W_use = Hd, Wd
    depth_use = depth_all
    conf_use = conf_all

    dense_dir = os.path.join(os.path.dirname(out_dir), 'dense')
    ...
    grid_ys, grid_xs = np.meshgrid(np.arange(0, H_use, STRIDE_DENSE),
                                    np.arange(0, W_use, STRIDE_DENSE), indexing='ij')
    grid_ys = grid_ys.ravel(); grid_xs = grid_xs.ravel()

    next_pid = 1; new_pts = {}; new_imgs = {}
    from PIL import Image as PILImg
    cam0_id = list(cameras_in.keys())[0]
    # Add ALL images with optimized poses
    for idx in range(N):
        img_id = img_items[idx][0]
        if recon.exists_image(img_id) and recon.image(img_id).has_pose:
            cfw = recon.image(img_id).cam_from_world()
            R = cfw.rotation.matrix(); tvec = cfw.translation
        else:
            R = ext_all[idx,:3,:3]; tvec = ext_all[idx,:3,3]
        qvec = colmap_utils.rotmat2qvec(R.astype(np.float64))
        new_imgs[img_id] = colmap_utils.Image(
            id=img_id, qvec=qvec, tvec=tvec.astype(np.float64),
            camera_id=cam0_id, name=img_names[idx],
            xys=np.zeros((0,2), dtype=np.float64), point3D_ids=np.zeros(0, dtype=np.int64))
    # Generate dense points from selected images
    for i in tqdm(selected, desc='Dense unproject'):
        if recon.exists_image(i + 1) and recon.image(i + 1).has_pose:
            cfw = recon.image(i + 1).cam_from_world()
            R = cfw.rotation.matrix(); tvec = cfw.translation
        else:
            R = ext_all[i,:3,:3]; tvec = ext_all[i,:3,3]
        Ki = K_use[i]
        dv = depth_use[i, grid_ys, grid_xs]; cv = conf_use[i, grid_ys, grid_xs]
        valid = (dv > 0) & (cv > CONF_THRESH)
        ys, xs = grid_ys[valid], grid_xs[valid]; dv = dv[valid]
        if len(xs) == 0: continue
        fx, fy, cx, cy = Ki[0,0], Ki[1,1], Ki[0,2], Ki[1,2]
        x_cam = (xs.astype(np.float32) - cx) / fx * dv
        y_cam = (ys.astype(np.float32) - cy) / fy * dv
        Xc = np.stack([x_cam, y_cam, dv], axis=1)
        X_w = (R @ Xc.T + tvec.reshape(3,1)).T
        img_full = np.array(PILImg.open(str(img_dir / img_names[i])).convert('RGB'))
        sy_i, sx_i = img_full.shape[0]/H_use, img_full.shape[1]/W_use
        y_img = np.round(ys*sy_i).astype(int).clip(0, img_full.shape[0]-1)
        x_img = np.round(xs*sx_i).astype(int).clip(0, img_full.shape[1]-1)
        colors = img_full[y_img, x_img]
        n_pts = len(xs); pids = np.arange(next_pid, next_pid + n_pts); next_pid += n_pts
        img_id = img_items[i][0]
        for k in range(n_pts):
            new_pts[int(pids[k])] = colmap_utils.Point3D(
                id=int(pids[k]), xyz=X_w[k].astype(np.float64),
                rgb=colors[k].astype(np.uint8), error=np.float64(0.0),
                image_ids=np.array([img_id]), point2D_idxs=np.array([k]))
        qvec = colmap_utils.rotmat2qvec(R.astype(np.float64))
        xys_out = np.stack([xs.astype(np.float64)*sx_i, ys.astype(np.float64)*sy_i], axis=1)
        new_imgs[img_id] = colmap_utils.Image(
            id=img_id, qvec=qvec, tvec=tvec.astype(np.float64),
            camera_id=cam0_id, name=img_names[i],
            xys=xys_out.astype(np.float64), point3D_ids=pids.astype(np.int64))
    colmap_utils.write_cameras_binary(new_cameras, os.path.join(dense_dir, 'cameras.bin'))
    colmap_utils.write_images_binary(new_imgs, os.path.join(dense_dir, 'images.bin'))
    colmap_utils.write_points3D_binary(new_pts, os.path.join(dense_dir, 'points3D.bin'))
    print(f"Saved dense cloud: {len(new_pts)} points from {len(selected)} images → {dense_dir}/")
